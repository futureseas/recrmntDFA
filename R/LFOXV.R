...
LFOXV <- function(dfaDat, # data matrix formatted for MARSS input (variables in rows, time in columns)
                  ...
                  horizon = 1, # number of forecast years to predict out-of-sample residuals for. Default is 1 yr
  ...
  for(i in 1:peels){
    ...
    resids <- OSAResids(objMARSS = peelDFA, fullDat = dfaDat, p = i, horizon = horizon,
                        colsRMSE = colsRMSE)
  ...
  overPeel <- peelRMSE %>% 
                  ...
                  group_by(.rownames, resType, predHoriz) %>%
  ...
  overPeelthenVar <- overPeel %>% group_by(resType, predHoriz) %>%
  ...
  #                                  values_to = "resids", names_to = "resType") %>%
  #                     # Calc RMSE over all time points
  #                     group_by(peel, resType, predHoriz) %>%
  #                     summarize(sosRes = sum(resids^2, na.rm = TRUE),
  #                               nObs = sum(!is.na(resids)),
  #                               avgAbsRes = mean(abs(resids), na.rm = TRUE)) %>%
  #                     mutate(RMSE = sqrt(sosRes/nObs)) %>% arrange(RMSE) %>%
  #                     group_by(resType, predHoriz) %>%
  #                     summarize(totRMSE = sum(RMSE),
  #                               meanRMSE = mean(RMSE))
  ...
  overPeelandVar <- overPeelandVar %>% mutate(compMethod = "peel & variable combined",
                                              variable = "total") %>% 
                        select(compMethod, variable, resType, predHoriz, RMSE)
