...
waaFishery <- read_csv(paste0(datPath, "anchovySSfiles/anchovy_formatted_wtatage_for_Robert.csv"))
...
sardBio %>% ggplot(aes(x = Yr, y = sardBioSmrySeas1)) +
  geom_line()
...
anchBio %>% ggplot(aes(x = Yr, y = anchBioSmrySeas1)) +
  geom_line()
...
waaFishery <- waaFishery %>% filter(Seas == 1) %>%
                select(modelyear, a1) %>%
                rename(year = modelyear,
                       age1SprAnchmeanWAA = a1)
...
outSS3 <- sardBio %>% full_join(sardRec, anchBio, by = "Yr")  %>% 
            full_join(anchBio, by = "Yr") %>% 
            full_join(anchRec, by = "Yr") %>%
            rename(year = Yr)
